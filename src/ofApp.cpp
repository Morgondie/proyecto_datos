...
void ofApp::setupLemmings()
{
	...
	radioCirculo = 20;
	//cargar spritesheet
	playerSpriteImg.load("cream.png");
	//tamaño del jugador
	...
	*/
	...
	//items
	srand(time(NULL));
	item1x = rand() % ofGetWidth() - 200;
	item1y = rand() % ofGetHeight() - 200;
	espada.load("espada.png");
	item1Size = ofVec2f(32, 48);
	item1Offset = ofVec2f(2, 4);

	item2x = rand() % ofGetWidth() - 100;
	item2y = rand() % ofGetHeight() - 100;
	beneno.load("veneno.png");
	item2Size = ofVec2f(32, 48);
	item2Offset = ofVec2f(2, 4);
	/*
	item3x = rand() % ofGetWidth() - 100;
	item3y = rand() % ofGetHeight() - 100;
	beneno.load("arbol.png");
	item3Size = ofVec2f(32, 48);
...
void ofApp::draw()
{
	...
	else if (appstate == EAppState::lemmings)
	{
		...
		beneno.drawSubsection(item2x, item2y, item2Size.x, item2Size.y, item2Offset.x, item2Offset.y);
